...
export function TpoProfileClient({ userProfile, initialData }: Props) {
    ...
    startTransition(async () => {
      try {
        if (section === "account") {
          ...
          if (error) {
            console.error("TPO professional upsert error:", error)
  ...
  async function handleCroppedAvatarUpload(croppedFile: File) {
    ...
    try {
      ...
      const newPath = `TPO/${userProfile.id}/avatar/${timestamp}.jpg`
  ...
  return (
    <div className="flex flex-col gap-6 px-4 py-8 md:px-8">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <p className="text-sm text-muted-foreground">Manage your TPO profile details</p>
      </div>
            ...
            {editing("profile") ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
                ...
                <div className="space-y-0.5">
                  ...
                  <div className="text-sm font-medium">TPO</div>
                </div>
              </div>
            )}
          </CardContent>
