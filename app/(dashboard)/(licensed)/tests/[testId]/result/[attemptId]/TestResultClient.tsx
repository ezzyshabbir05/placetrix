        ...
        <>
          ...
          <div className="rounded-xl border bg-card p-4 sm:p-5 space-y-4 min-w-0 overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 min-w-0">
              ...
              {(attempt.active_time_taken != null || attempt.total_time_taken != null || attempt.tab_switch_count != null) && (
                <div className="flex flex-wrap items-center sm:items-end gap-2 shrink-0">
                  {attempt.active_time_taken != null && (
                    <div className="flex items-center gap-1.5 rounded-lg bg-muted/50 px-2.5 py-1.5 text-xs font-medium text-muted-foreground" title="Total active time spent on questions">
                      ...
                      <span>Active: <span className="tabular-nums font-semibold text-foreground">{formatSeconds(attempt.active_time_taken)}</span></span>
                    </div>
                  )}
                  {(attempt.total_time_taken != null || (attempt.submitted_at && attempt.started_at)) && (
                    <div className="flex items-center gap-1.5 rounded-lg bg-muted/50 px-2.5 py-1.5 text-xs font-medium text-muted-foreground" title="Total duration from start to submission">
                      ...
                      <span>Total: <span className="tabular-nums font-semibold text-foreground">{formatSeconds(attempt.total_time_taken ?? (attempt.submitted_at && attempt.started_at ? Math.max(0, Math.round((new Date(attempt.submitted_at).getTime() - new Date(attempt.started_at).getTime()) / 1000)) : null))}</span></span>
                    </div>
                  )}
